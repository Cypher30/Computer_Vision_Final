...
from utils import build_imagenet, save_checkpoint
...
def main():
    ...
    # build dataloader
    root = os.environ.get('DATA', './data/imagenet')
    train_dataloader, test_dataloader = build_imagenet(gpc.config.BATCH_SIZE, root, pad_if_needed=True)
    ...
    save_checkpoint(model.mlp_head, filename='mlp_imagenet.pth.tar')
